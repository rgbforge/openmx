...
)
{
  ...
  *output_hks = TRAN_output_hks;
  ...
  input_int("NEGF.SCF.Iter.Band", &TRAN_SCF_Iter_Band, 10); 

  /* mixing weight for Hamiltonian near the boundary between the C and L1 (R1) */

  input_double("NEGF.Mixing.H.Boundary", &TRAN_mixweight_H_at_boundary, -1.0); 

  /* whether fix the chemical potential during the band calculation or not. */

  s_vec[0]="fix"; s_vec[1]="free"; 
  i_vec[0]=0    ; i_vec[1]=1    ;
  input_string2int("NEGF.ChemP.Band", &TRAN_ChemP_Band, 2, s_vec,i_vec);

  /* mix the Hartree potential during the transition from Band to NEGF. */

  s_vec[0]="on"; s_vec[1]="off"; 
  i_vec[0]=1    ; i_vec[1]=0    ;
  input_string2int("NEGF.Band2NEGF.Hartree", &TRAN_Band2NEGF_Hartree, 2, s_vec,i_vec);
  ...
  input_logical("NEGF.NonSCF.bias",&tran_NonSCF_bias,0); /* default=off */
  ...
  if ( tran_bias_apply ) {
    ...
    */
    ...
    tran_biasvoltage_e[0] =-0.5*tmp/TRAN_eV2Hartree; 
    tran_biasvoltage_e[1] = 0.5*tmp/TRAN_eV2Hartree; 
