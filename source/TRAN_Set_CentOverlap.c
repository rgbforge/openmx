...
{ 
  ...
  double coe,x0,x1,xa,xb,xc,bias;
  ...
  if (0.0<=(TRAN_mixweight_H_at_boundary+1.0e-14)){
    H_mixing_flag = 1;
  ...
  if ((job&1)==1) {
	...
	for (i=0; i<tnoA; i++){
	  for (j=0; j<tnoB; j++){
		...
		for (k=0; k<=SpinP_switch; k++) {
		  HCC[k][SCC_ref(Anum+i,Bnum+j)].r += co*S1[q]*bias;
		  HCC[k][SCC_ref(Anum+i,Bnum+j)].i += si*S1[q]*bias;
		}
	      }
	    }
	...
	for (GB_AN=1; GB_AN<=atomnum; GB_AN++){
	  ...
	  if ( TRAN_region[GA_AN]==12 && TRAN_region[GB_AN]==12 ) {
	    ...
	    for (i=0; i<tnoA; i++){
		  ...
		  HCC[k][SCC_ref(Anum+i,Bnum+j)].r = coe*HCC[k][SCC_ref(Anum+i,Bnum+j)].r + (1.0-coe)*H00_e[iside][k][S00l_ref(Anum_e+i,Bnum_e+j)].r;
		  HCC[k][SCC_ref(Anum+i,Bnum+j)].i = coe*HCC[k][SCC_ref(Anum+i,Bnum+j)].i + (1.0-coe)*H00_e[iside][k][S00l_ref(Anum_e+i,Bnum_e+j)].i;
	...
	for (GB_AN=1; GB_AN<=atomnum; GB_AN++){
	  ...
	  if ( TRAN_region[GA_AN]==13 && TRAN_region[GB_AN]==13 )  {
		...
		for (k=0; k<=SpinP_switch; k++) {
		  ...
		  HCC[k][SCC_ref(Anum+i,Bnum+j)].r = coe*HCC[k][SCC_ref(Anum+i,Bnum+j)].r + (1.0-coe)*H00_e[iside][k][S00r_ref(Anum_e+i,Bnum_e+j)].r;
		  HCC[k][SCC_ref(Anum+i,Bnum+j)].i = coe*HCC[k][SCC_ref(Anum+i,Bnum+j)].i + (1.0-coe)*H00_e[iside][k][S00r_ref(Anum_e+i,Bnum_e+j)].i;
          ...
          for (i=0; i<tnoA; i++){
            for (j=0; j<tnoB_e; j++){
              ...
              for (k=0; k<=SpinP_switch; k++) {
                ...
                HCL[k][SCL_ref(Anum+i,Bnum_e+j)].r = H01_e[iside][k][ S00l_ref(Anum_e+i, Bnum_e+j)].r;
                HCL[k][SCL_ref(Anum+i,Bnum_e+j)].i = H01_e[iside][k][ S00l_ref(Anum_e+i, Bnum_e+j)].i;
              }
            }
          }
	}
          ...
          for (i=0; i<tnoA; i++){
            for (j=0; j<tnoB_e; j++){
              ...
              for (k=0; k<=SpinP_switch; k++) {
                ...
                HCR[k][SCR_ref(Anum+i,Bnum_e+j)].r = H01_e[iside][k][ S00r_ref(Anum_e+i, Bnum_e+j)].r;
                HCR[k][SCR_ref(Anum+i,Bnum_e+j)].i = H01_e[iside][k][ S00r_ref(Anum_e+i, Bnum_e+j)].i;
