...
{ 
  ...
  if (0.0<TRAN_mixweight_H_at_boundary){
    H_mixing_flag = 1;
    coe = TRAN_mixweight_H_at_boundary;
  }
  else{
    H_mixing_flag = 0;
  }
  ...
  if ((job&1)==1) {
	...
	Bnum = MP[GB_AN];
        ...
        xb = Gxyz[GB_AN][ip];
        xc = 0.5*(xa + xb);
        bias = (tran_biasvoltage_e[1]-tran_biasvoltage_e[0])/(x1-x0)*(xc-x0) + tran_biasvoltage_e[0];
	...
	for (i=0; i<tnoA; i++){
	  for (j=0; j<tnoB; j++){
	      ...
	      HCC_nc[SCC11_ref(Anum+i,Bnum+j)].i += co*b11 + si*a11;
              ...
              if (tran_NonSCF_bias==1){

		/* H_alpha-alpha */
		HCC_nc[SCC00_ref(Anum+i,Bnum+j)].r += co*S1[q]*bias;
		HCC_nc[SCC00_ref(Anum+i,Bnum+j)].i += si*S1[q]*bias;

		/* H_beta-beta */
		HCC_nc[SCC11_ref(Anum+i,Bnum+j)].r += co*S1[q]*bias;
		HCC_nc[SCC11_ref(Anum+i,Bnum+j)].i += si*S1[q]*bias;
	      }
	    }
  ...
  if ( (job&2) == 2 ) {
	...
	for (GB_AN=1; GB_AN<=atomnum; GB_AN++){
          ...
          if (H_mixing_flag==0) coe = 0.5*(tanh(0.5*(fabs(xc-x0)-2.0))+1.0);
	  ...
	  if ( TRAN_region[GA_AN]==12 && TRAN_region[GB_AN]==12 ) {
		...
		/* H_alpha-alpha */
		HCC_nc[SCC00_ref(Anum+i,Bnum+j)].r = coe*HCC_nc[SCC00_ref(Anum+i,Bnum+j)].r + (1.0-coe)*H00_nc_e[iside][S00l00_ref(Anum_e+i,Bnum_e+j)].r;
		/* H_alpha-beta */
		HCC_nc[SCC01_ref(Anum+i,Bnum+j)].r = coe*HCC_nc[SCC01_ref(Anum+i,Bnum+j)].r + (1.0-coe)*H00_nc_e[iside][S00l01_ref(Anum_e+i,Bnum_e+j)].r; 
		/* H_beta-alpha */
		HCC_nc[SCC10_ref(Anum+i,Bnum+j)].r = coe*HCC_nc[SCC10_ref(Anum+i,Bnum+j)].r + (1.0-coe)*H00_nc_e[iside][S00l10_ref(Anum_e+i,Bnum_e+j)].r; 
		/* H_beta-beta */
		HCC_nc[SCC11_ref(Anum+i,Bnum+j)].r = coe*HCC_nc[SCC11_ref(Anum+i,Bnum+j)].r + (1.0-coe)*H00_nc_e[iside][S00l11_ref(Anum_e+i,Bnum_e+j)].r;
		...
		/* H_alpha-alpha */
		HCC_nc[SCC00_ref(Anum+i,Bnum+j)].i = coe*HCC_nc[SCC00_ref(Anum+i,Bnum+j)].i + (1.0-coe)*H00_nc_e[iside][S00l00_ref(Anum_e+i,Bnum_e+j)].i;
		/* H_alpha-beta */
		HCC_nc[SCC01_ref(Anum+i,Bnum+j)].i = coe*HCC_nc[SCC01_ref(Anum+i,Bnum+j)].i + (1.0-coe)*H00_nc_e[iside][S00l01_ref(Anum_e+i,Bnum_e+j)].i; 
		/* H_beta-alpha */
		HCC_nc[SCC10_ref(Anum+i,Bnum+j)].i = coe*HCC_nc[SCC10_ref(Anum+i,Bnum+j)].i + (1.0-coe)*H00_nc_e[iside][S00l10_ref(Anum_e+i,Bnum_e+j)].i; 
		/* H_beta-beta */
		HCC_nc[SCC11_ref(Anum+i,Bnum+j)].i = coe*HCC_nc[SCC11_ref(Anum+i,Bnum+j)].i + (1.0-coe)*H00_nc_e[iside][S00l11_ref(Anum_e+i,Bnum_e+j)].i;
	...
	for (GB_AN=1; GB_AN<=atomnum; GB_AN++){
          ...
          xb = Gxyz[GB_AN][ip];
          xc = 0.5*(xa + xb);
          if (H_mixing_flag==0) coe = 0.5*(tanh(0.5*(fabs(xc-x1)-2.0))+1.0);
	  ...
	  if ( TRAN_region[GA_AN]==13 && TRAN_region[GB_AN]==13 )  {
		...
		/* H_alpha-alpha */
		HCC_nc[SCC00_ref(Anum+i,Bnum+j)].r = coe*HCC_nc[SCC00_ref(Anum+i,Bnum+j)].r + (1.0-coe)*H00_nc_e[iside][S00r00_ref(Anum_e+i,Bnum_e+j)].r;
		/* H_alpha-beta */
		HCC_nc[SCC01_ref(Anum+i,Bnum+j)].r = coe*HCC_nc[SCC01_ref(Anum+i,Bnum+j)].r + (1.0-coe)*H00_nc_e[iside][S00r01_ref(Anum_e+i,Bnum_e+j)].r; 
		/* H_beta-alpha */
		HCC_nc[SCC10_ref(Anum+i,Bnum+j)].r = coe*HCC_nc[SCC10_ref(Anum+i,Bnum+j)].r + (1.0-coe)*H00_nc_e[iside][S00r10_ref(Anum_e+i,Bnum_e+j)].r; 
		/* H_beta-beta */
		HCC_nc[SCC11_ref(Anum+i,Bnum+j)].r = coe*HCC_nc[SCC11_ref(Anum+i,Bnum+j)].r + (1.0-coe)*H00_nc_e[iside][S00r11_ref(Anum_e+i,Bnum_e+j)].r;
		...
		/* H_alpha-alpha */
		HCC_nc[SCC00_ref(Anum+i,Bnum+j)].i = coe*HCC_nc[SCC00_ref(Anum+i,Bnum+j)].i + (1.0-coe)*H00_nc_e[iside][S00r00_ref(Anum_e+i,Bnum_e+j)].i;
		/* H_alpha-beta */
		HCC_nc[SCC01_ref(Anum+i,Bnum+j)].i = coe*HCC_nc[SCC01_ref(Anum+i,Bnum+j)].i + (1.0-coe)*H00_nc_e[iside][S00r01_ref(Anum_e+i,Bnum_e+j)].i; 
		/* H_beta-alpha */
		HCC_nc[SCC10_ref(Anum+i,Bnum+j)].i = coe*HCC_nc[SCC10_ref(Anum+i,Bnum+j)].i + (1.0-coe)*H00_nc_e[iside][S00r10_ref(Anum_e+i,Bnum_e+j)].i; 
		/* H_beta-beta */
		HCC_nc[SCC11_ref(Anum+i,Bnum+j)].i = coe*HCC_nc[SCC11_ref(Anum+i,Bnum+j)].i + (1.0-coe)*H00_nc_e[iside][S00r11_ref(Anum_e+i,Bnum_e+j)].i;
